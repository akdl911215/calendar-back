...
import { UsersEmailDuplicateVerificationInputDto } from './dtos/users.duplicate.verification.dto';
import { jestErrorHandling } from '../_common/dtos/jest.error.handling';
import { BadRequestException } from '@nestjs/common';
import { DATE } from '../_common/dtos/get.date';
import { EMAIL_REQUIRED } from '../_common/https/errors/400';
...
describe('Users Email Duplicate Verification Process', () => {
  ...
  describe('user email duplicate verification unit test', () => {
    it('email empty, so it fails', async () => {
      const dto: UsersEmailDuplicateVerificationInputDto = {
        email: '',
      };

      try {
        await service.emailDuplicateVerification(dto);
      } catch (e: any) {
    ...
    it('success should user duplicate verification', async () => {
      ...
      try {
        const { emailExists } = await service.emailDuplicateVerification(dto);
        expect(emailExists).toStrictEqual(true);
